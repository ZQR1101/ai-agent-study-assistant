...
LangGraph Runtime 使用 `StateGraph`。当前节点包括 planner、rag、chat、explain、summarize、flashcard、quiz 和 finalizer。条件路由根据 intent 与计划决定下一个节点。节点仍复用同一 Tool Registry，而不是维护第二套工具实现。
